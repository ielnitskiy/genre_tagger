...
from .genrelists import fingerprint, load_aliases, load_banlist, migrate_json_if_needed
...
# Версия конвейера фильтрации. Меняется вместе с логикой _filter_tags, чтобы
# обновление кода форсировало офлайн-пересчёт уже обработанной библиотеки.
PIPELINE_VERSION = "2"
...
def _rewrite_on_config_change(cache: Cache, lastfm: LastfmClient, config_hash: str) -> None:
    """Если banlist.txt/aliases.txt изменились с прошлого запуска, пересчитываем
    жанры локально из уже сохранённого raw_response (без обращения к Last.fm) и
    форсируем перезапись ID3 у всех затронутых артистов на следующем run_once.
    Если после пересчёта у артиста не осталось ни одного жанра (все попали в
    бан-лист), genre в кэше станет None, а scan_artist/_strip_genre_from_files
    снимет тег с уже затегированных файлов."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Показать, какие жанры сейчас стоят в библиотеке (по числу треков) "
            "и какие из них похожи по написанию. Ничего не меняет и не ходит в "
            "сеть — это материал для правки banlist.txt и aliases.txt."
        ),
    )
    ...
    if args.report:
        print_report(cache, config.banlist_path, config.aliases_path)
        return

    # Разовая конвертация старых genre_banlist.json/genre_aliases.json в .txt.
    migrate_json_if_needed(config.banlist_path, config.aliases_path)
    banned = load_banlist(config.banlist_path)
    aliases = load_aliases(config.aliases_path, banned)
    lastfm = LastfmClient(config.lastfm_api_key, aliases=aliases, banned=banned)
    ...
    _rewrite_on_config_change(cache, lastfm, f"{PIPELINE_VERSION}:{fingerprint(banned, aliases)}")
