...
def main() -> None:
    ...
    if not args.no_clusters:
        # Поиск кандидатов на --add-alias — только имена тегов, без весов и
        # без числа треков (это отдельная от бан-листа задача, обсудим формат
        # словаря алиасов отдельно). --min-count здесь используется только
        # чтобы убрать совсем случайный мусор (опечатки одного пользователя
        # Last.fm) из пула сравнения — сам вес нигде не печатается.
        cluster_pool = [tag for tag, count in totals.items() if count >= args.min_count]
